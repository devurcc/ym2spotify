...
- За один запрос в плейлист добавляются **до 10** трек-URI подряду (до 100 разрешены API); малые батчи раньше выявляют сбои. Порядок как в текстовом файле **сверху вниз** сохраняется: первая найденная строка оказывается **выше** следующих — в каждом `POST …/items` передаётся `position`.
